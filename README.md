...
This repository is a rebuild of <a href="https://github.com/lucidrains/vit-pytorch">lucidrains/vit-pytorch</a> (MIT, Copyright (c) 2020 Phil Wang), based on upstream commit <code>c362297</code>. The original license is preserved in [LICENSE](./LICENSE).

It is repackaged and maintained here as a <a href="https://docs.astral.sh/uv/">uv</a>-managed project, with these changes:

- installation, locking and task running go through uv (`uv sync`, `uv run`)
- `requires-python` raised from `>=3.8` to `>=3.9`: a core dependency (`einops>=0.8.2`) requires Python >=3.9, so the previous metadata was self-contradictory and made `uv sync` fail to resolve
- removed the stale `torch==2.5.0` / `torchvision==0.20.0` pins from the `test` extra, which forced a downgrade to 2024-era CUDA builds
- added a `dev` dependency group (`pytest`, `accelerate`, `wandb`) so a fresh `uv sync` can run the test suite and the CIFAR100 example — the same group also carries what `examples/cats_and_dogs.ipynb` imports (`ipykernel`, `ipywidgets`, `matplotlib`, `pandas`, `scikit-learn`, `linformer`)
- removed the PEP 723 inline script metadata from `train_vit_decorr.py`, which pinned the PyPI `vit-pytorch` rather than this checkout
- `uv.lock` is resolved against the Huawei Cloud PyPI mirror (`repo.huaweicloud.com`), which is fast from mainland China; regenerate it with `uv lock --default-index https://pypi.org/simple` if you are elsewhere
...
$ uv run train_vit_decorr.py  # CIFAR100 example (~170MB downloaded into ./data)
```

### Running the example notebook

`ipykernel` is part of the dev group, so this project environment can be used directly as a Jupyter kernel. In VS Code, open this folder and pick **Select Kernel → Python Environments → `./.venv/bin/python`**.
